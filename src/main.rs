...
impl LoginPlayer {
    ...
    pub fn state(&self) -> LoginPlayerState {
        self.state
    }
...
impl Player {
    pub fn initialize(login_player: LoginPlayer) -> Result<Self> {
        ...
        let mut connection = login_player.into_connection();

        connection.send(packet::play::SynchronizePlayerPosition {
            relative: false,
            x: Some(0.0),
            y: Some(0.0),
            z: Some(0.0),
        ...
        connection.send(packet::play::GameEvent {
            event: 13,
            value: 0.0,
        })?;
...
impl Server {
    ...
    pub fn handle_login_players(&mut self) -> Result<()> {
        ...
        for i in (0..self.login_players.len()).rev() {
            if self.login_players[i].state() == LoginPlayerState::Play {
                let login_player = self.login_players.remove(i);
                let player = Player::initialize(login_player)?;
